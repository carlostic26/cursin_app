...
class _percentIndicatorState extends State<percentIndicator> {
  ...
  // Después de 9 segundos se desactiva el conteo de cursos
  Future<void> waitCountText() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    contadorFinalizado = prefs.getBool('countedCourses') ?? false;

    await Future.delayed(const Duration(seconds: 9), () {
      prefs.setBool('countedCourses', true);
    });
  }

  // Después de 10 segundos se activa el botón de continuar
  Future<void> waitAndEnableButton() async {
    await Future.delayed(const Duration(seconds: 10));
    setState(() {
      isButtonVisible = true; // Mostrar el botón después de 10 segundos
    });
  }
              ...
              children: [
                Text("Encontrando ",
                    style: new TextStyle(
                        ...
                        color: Colors.white)),
                if (_isFirstBuild) // Verificar si es la primera construcción
                  McCountingText(
                    begin: 0,
